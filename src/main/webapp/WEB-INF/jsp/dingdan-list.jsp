...
<section class="Hui-article-box">
	...
	<div class="Hui-article">
		<article class="cl pd-20">
			<div class="text-c">
				<form class="Huiform" method="post" action="${pageContext.request.contextPath }/orders/findOrdersLike" target="_self">
					<input type="text" class="input-text" style="width:250px" placeholder="输入车位编号或姓名" name="searchorders" value="${searchorders }">
					<button type="submit" class="btn btn-success"><i class="Hui-iconfont">&#xe665;</i> 搜订单</button>
				</form>
			</div>
			<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="r">共有数据：<strong id="count">${count }</strong> 条</span> </div>
			<table class="table table-border table-bordered table-bg">
				<thead>
					...
					<c:forEach items="${ordersList}" var="orders" varStatus="status">
						<tr class="tohidden" id=${orders.id }>
							<td>${status.index+1 }</td>
							<td>${orders.code}</td>
							<td>${orders.user.name}</td>
							<td>${orders.user.phone}</td>
							<td>${orders.createdate}</td>
							<td>${orders.user.plateNum}</td>
							<td>${orders.park.name}</td>
							<td>${orders.total}</td>
							<td>
								<c:if test="${orders.status==1}"><span class="label label-success radius">订单完成</span></c:if>
								<c:if test="${orders.status==0}"><span class="label radius">待付款</span></c:if>
							</td>
							<td class="td-manage">
								<a class="ml-5" href="javascript:void(0)" onclick="deleteById(this)" style="text-decoration: none"
...
/* 				function searchOrders(){
				    var value=$('#searchorders').val();
					var num=0;
					$('.tohidden').css("display","none");
					$.getJSON("${pageContext.request.contextPath}/orders/findOrdersLike",{name:value,plateNum:value},function(data){
						$.each(data,function(i,item){
							num++;
							$('tbody').append('<tr class="tohidden"><td>'+(i+1)+'</td><td>'+item.code+'</td><td>'+item.user.name+'</td><td>'+item.user.phone+'</td><td>'+item.createdate+'</td><td>'+item.user.plateNum+'</td><td>'+item.park.name+'</td><td>'+item.total+'</td><td>'+item.status+'</td><td><a href="javascript:void(0)" onclick="deleteById('+item.id+')">删除</a></td></tr>');
						});
						$("#count").html(num);
					});
				} */
				function deleteById(obj){
					layer.confirm('确认要删除吗？', function(index){
						...
						$.ajax({
							type:'POST',
							url:'${pageContext.request.contextPath }/orders/deleteOrderById',
							data:{"id":id},
							dataType:'json',
