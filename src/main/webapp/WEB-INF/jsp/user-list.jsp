...
<body>
	...
	<div class="Hui-article">
		...
		<div class="text-c">
			<form class="Huiform" method="post"
				action="${pageContext.request.contextPath }/user/findUsers"
				target="_self">
				<input type="text" class="input-text" style="width: 250px"
					placeholder="输入用户名称" id="searchName" name="searchName"
					value="${searchName }">
		...
		<table class="table table-border table-bordered table-bg">
			...
			<tbody>
				<c:forEach items="${userList }" var="user" varStatus="status">
					<tr class="tohidden" id=${user.id }>
						<td>${status.index+1 }</td>
						<td>${user.name }</td>
						<td>${user.phone }</td>
						<td>${user.plateNum }</td>
						<td>${user.createdate }</td>
						<td class="td-status"><c:if test="${user.stauts==0 }">
								<span class="label label-success radius">已启用</span>
							</c:if> <c:if test="${user.stauts==1 }">
								<span class="label radius">已停用</span>
							</c:if></td>
						<td class="td-manage"><c:if test="${user.stauts==0 }">
								<a onClick="user_stop(this)" href="javascript:;" title="停用"
									style="text-decoration: none"><i class="Hui-iconfont">&#xe631;</i></a>
							</c:if> <c:if test="${user.stauts==1 }">
								<a onClick="user_start(this)" href="javascript:;" title="启用"
									style="text-decoration: none"><i class="Hui-iconfont">&#xe615;</i></a>
							</c:if></td>
					</tr>
				</c:forEach>
	...
	<!--请在下方写此页面业务相关的脚本-->
	<script type="text/javascript"
		src="${pageContext.request.contextPath}/resources/lib/jquery.validation/1.14.0/jquery.validate.js"></script>
	<script type="text/javascript"
		src="${pageContext.request.contextPath}/resources/lib/jquery.validation/1.14.0/validate-methods.js"></script>
	<script type="text/javascript"
		src="${pageContext.request.contextPath}/resources/lib/jquery.validation/1.14.0/messages_zh.js"></script>
		...
		layer
				.confirm(
						...
						function(index) {
							...
							var userId = $(obj).parents("tr").attr("id");
							$
									.ajax({
										type : 'POST',
										url : '${pageContext.request.contextPath }/user/stopUser',
										data : {
											"id" : userId
										},
										dataType : 'json',
										success : function(data) {
											if (data.res == 0) {
												$(obj)
														...
														.find(".td-manage")
														.prepend(
																'<a onClick="user_start(this)" href="javascript:;" title="启用" style="text-decoration:none"><i class="Hui-iconfont">&#xe615;</i></a>');
												$(obj)
														.parents("tr")
														.find(".td-status")
														.html(
				...
				.confirm(
						...
						function(index) {
							...
							$
									.ajax({
										type : 'POST',
										url : '${pageContext.request.contextPath}/user/startUser',
										data : {
											"id" : userId
										},
										dataType : 'json',
										success : function(data) {
											...
											if (data.res == 0) {
												$(obj)
														...
														.find(".td-manage")
														.prepend(
																'<a onClick="user_stop(this)" href="javascript:;" title="停用" style="text-decoration:none"><i class="Hui-iconfont">&#xe631;</i></a>');
												$(obj)
														.parents("tr")
														.find(".td-status")
														.html(
