...
<body>
	...
	投诉建议管理 <span class="c-gray en">&gt;</span> 留言列表 <a
	...
	<div class="Hui-article">
		...
		<div class="text-c">
			<form
				action="${pageContext.request.contextPath }/message/findMessages"
				class="Huiform" method="post" action="" target="_self">
				<input type="text" class="input-text" style="width: 250px"
					placeholder="输入留言内容" name="content" value="${content }">
				<button type="submit" class="btn btn-success">
					<i class="Hui-iconfont">&#xe665;</i> 搜留言
				</button>
			</form>
		</div>
		...
		<!-- <script>
			function search(){
				var value = $("#searchMessage").val();
				var num = 0;
				$(".tohidden").css("display","none");
				$.getJSON("${pageContext.request.contextPath }/message/findMessageLikeContent?content="+value,function(data){
					$.each(data, function(i,item){
						num++;
						var child =  '<tr class="tohidden"><td>'+num+'</td><td>'+item.content+'</td><td>'+item.createdate+'</td><td>'+item.userName+'</td><td>'+item.answer+'</td><td>'+item.answerdate+'</td><td>'+item.adminName+'</td></tr>';
						$("tbody").append(child);
					});
					$("#count").html(num);
				});
		...
		<table class="table table-border table-bordered table-bg">
			<thead>
				<tr>
					<th scope="col" colspan="8">留言列表</th>
				</tr>
				<tr class="text-c">
					<th width="50">序号</th>
			...
			<tbody>
				<c:forEach items="${msgList }" var="message" varStatus="status">
					<tr class="tohidden" id=${message.id }>
						<td>${status.index+1 }</td>
						<td>${message.content }</td>
						<td>${message.createdate }</td>
						<td>${message.userName }</td>
						<td>${message.answer }</td>
						<td>${message.answerdate }</td>
						<td>${message.adminName }</td>
						<td class="td-manage"><c:if test="${message.answer==null }">
								<a class="ml-5"
									onclick="answer_add(this,'留言回复','${pageContext.request.contextPath }/admin/answer-add','800','300')"
									style="text-decoration: none" title="回复留言"><i
									class="Hui-iconfont"></i></a>
							</c:if> <a class="ml-5" onclick="message_del(this)"
							style="text-decoration: none" title="删除"><i
...
<script>
	function answer_add(obj, title, url, w, h) {
		var messageId = $(obj).parents("tr").attr("id");
	...
	function message_del(obj) {
		layer.confirm('确认要删除吗？', function(index) {
			...
			$.ajax({
				type : 'POST',
				url : '${pageContext.request.contextPath }/message/delMessage',
				data : {
					"id" : messageId
				},
				dataType : 'json',
